...
**Current queue:** EMPTY. CTO picks next bounded lane.
---
**Task ID:** CODEQL-NON-CHANNELS-RATE-LIMITING-CLUSTER-03-USER-DATA-CRUD
**Status:** CLOSED — 2026-04-14. Product anchor advanced to `3d81f2c5ff3d6b53349ec0ae45c61be9ba226d10` (merge commit for PR #121). Retired nine js/missing-rate-limiting alerts (all fixed_at=2026-04-14T18:59:27Z): #59 `index.ts:1135` (GET /api/memory, read), #63 `index.ts:1508` (GET /api/settings, read), #64 `index.ts:1550` (POST /api/settings, write), #67 `index.ts:2985` (GET /api/file-library, read), #68 `index.ts:2987` (DELETE /api/file-library/:name, destructive), #69 `index.ts:3544` (GET /api/images, read), #70 `index.ts:4648` (GET /api/tasks, read), #71 `index.ts:4668` (GET /api/dnd, read), #72 `index.ts:4767` (GET /api/contacts, read). Single-file bounded edit in `apps/server/src/index.ts`. PR #121 required a second push: the first CI run failed at E2E Smoke Tests (Playwright) because the initial `ADMIN_LIGHT` (30/5min) bucket was too tight for polled GET endpoints under Playwright fan-out + Next dev HMR, producing a 429 cascade (evidence: run 24416602047, `expect(response.status()).toBe(200)` receiving 429). Same PR was corrected in-place by switching the seven READ endpoints to inline `{maxAttempts:600, windowMs:60_000, lockoutMs:60_000}` (commit `db04efa`); mutating endpoints (POST /api/settings, DELETE /api/file-library/:name) kept `ADMIN_MODERATE`. Same helper, same prelude shape, single-file boundary preserved. Second CI run: 13/13 green. Post-merge live CodeQL re-scan proved all nine alerts state=fixed. Cumulative non-channel lane status: 19/20 alerts retired (Cluster 01: 4, Cluster 02: 6, Cluster 03: 9). Cluster 04 (3D-print, 1 alert) is locked-lane excluded and is the sole remaining carryover.
**Current queue:** EMPTY. CTO picks next bounded lane.
---
